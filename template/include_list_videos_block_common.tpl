		...
		{{if $list_videos_hide_headline!='true'}}
			<div class="heading cfx">
				...
				{{if $list_videos_show_sorting=='true'}}
					{{if count($data)>0 || $smarty.get.countryId|default:0 > 0}}
												...
												<nav>
													<ul class="drop-inner">
														{{foreach from=$lang.videos.sortings_top_rated item="item2"}}
															{{if $sort_by!=$item2}}
																<li>
																	<a class="js-filters" href="{{$base_url}}?by={{$item2}}">{{$lang.videos.list_sorting_period[$item2]}}</a>
																</li>
															{{/if}}
														{{/foreach}}
													</ul>
							...
							<div class="dropdown__block__menu" id="country_drop_{{$block_uid}}">
								<nav class="wide">
									<ul class="drop-inner">
										{{if $countryId}}
											<li>
												<a href="{{$lang.urls.memberzone_my_fav_videos}}">Country</a>
											</li>
										{{/if}}
										{{foreach from=$data_country item="item" key="key"}}
											{{if $countryId!=$item}}
												<li>
													<a href="{{$lang.urls.memberzone_my_fav_videos}}?countryId={{$item}}">{{$list_countries.name[$item]}}<img src="/static/images/flags/{{$list_countries.code[$item]|upper}}.png"/></a>
												</li>
											{{/if}}
										{{/foreach}}
									</ul>
								...
								<nav class="wide">
									<ul class="drop-inner">
										{{foreach item="item" from=$playlists}}
											<li>
												<a class="js-filters" href="{{$lang.urls.memberzone_my_playlist|replace:"%ID%":$item.playlist_id}}" {{if $playlist_id==$item.playlist_id}}active{{/if}}">{{$item.title}} ({{$item.total_videos}})</a>
											</li>
										{{/foreach}}
										<li>
