						...
						<li>
							{{if $lang.enable_subscriptions=='true' && $lang.enable_memberzone=='true'}}
								{{if $smarty.session.user_id<1}}
									<a href="{{$lang.urls.login}}" data-action="popup" class="button btn btn-custom">
										<span class="text">{{$lang.channels.info_action_subscribe}}</span>
										<span class="button-info value">{{$data.user.dvds.subscribers_count|traffic_format}}</span>
									</a>
								{{else}}
									{{if $data.user.dvds.is_subscribed==1}}
										<a href="#unsubscribe" class="button btn btn-custom" data-unsubscribe-to="dvd" data-id="{{$data.user.dvds.dvd_id}}">
											<span class="text">{{$lang.channels.info_action_unsubscribe}}</span>
											<span class="button-info value">{{$data.user.dvds.subscribers_count|traffic_format}}</span>
										</a>
									{{else}}
										<a href="#subscribe" class="button btn btn-custom" data-subscribe-to="dvd" data-id="{{$data.user.dvds.dvd_id}}">
											<span class="text">{{$lang.channels.info_action_subscribe}}</span>
											<span class="button-info value">{{$data.user.dvds.subscribers_count|traffic_format}}</span>
										</a>
									{{/if}}
      ...
      <div class="rate-wrapper">
        ...
        <div class="col">
          <div class="item">
			...
			{{/if}}
			{{if $can_favourite==1}}
				<a title="{{$lang.videos.video_details_action_add_to_favourites[0]}}" href="#add2fav" class="info-bar__button link fav {{if $data.is_favourited==1}}subscribed{{/if}}" data-action="drop" data-drop-id="fav_list">
			...
			{{else}}
				<a title="{{$lang.videos.video_details_action_add_to_favourites[0]}}" href="{{if $smarty.session.user_id>0}}{{$lang.urls.upgrade_required}}{{else}}{{$lang.urls.login_required}}{{/if}}" data-action="popup" class="info-bar__button link fav">
          ...
          <div class="item">
			...
			{{/if}}
            <a href="#" class="link chat info-bar__button" title="{{$lang.comments.btn_add_comment}}" {{if $can_add_comment==1}}href="#add_comment" data-action="toggle" data-toggle-id="new_comment"{{else}}href="{{if $smarty.session.status_id>1}}{{$lang.urls.upgrade_required}}{{else}}{{$lang.urls.login_required}}{{/if}}" data-action="popup"{{/if}}>
              <i class="icon-dialog-clouds-shape-19"></i><span>{{$data.comments_count}}</span></i>
            </a>
          </div>
        </div>
      </div>
