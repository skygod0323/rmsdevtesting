<?php
function model_commentsShow($block_config,$object_id)
{
	...
	$model_info=mr2array_single($result);
	$model_id=intval($model_info['model_id']);
	...
	if (isset($block_config['var_from']))
	{
		...
		$total_count=mr2number(sql_pr("select count(*) from $config[tables_prefix]comments where object_id=$model_id and object_type_id=4 and is_approved=1 and added_date<=? $where",date("Y-m-d H:i:s")));
		...
		$data=mr2array(sql_pr("select $config[tables_prefix]comments.*, $config[tables_prefix]users.status_id, $config[tables_prefix]users.avatar, $config[tables_prefix]users.username, $config[tables_prefix]users.display_name, $config[tables_prefix]users.gender_id, $config[tables_prefix]users.birth_date, $config[tables_prefix]users.country_id, $config[tables_prefix]users.city from $config[tables_prefix]comments left join $config[tables_prefix]users on $config[tables_prefix]comments.user_id=$config[tables_prefix]users.user_id where $config[tables_prefix]comments.object_id=? and $config[tables_prefix]comments.object_type_id=4 and $config[tables_prefix]comments.is_approved=1 and $config[tables_prefix]comments.added_date<=? $where order by $config[tables_prefix]comments.$sort_by $limit",$model_id,date("Y-m-d H:i:s")));
	...
	$smarty->assign("model_info",$model_info);
...
function model_commentsGetHash($block_config)
{
	...
	{
		$current_version=get_block_version('comments_info','model',$id,$dir);
	}
...
function model_commentsCacheControl($block_config)
{
	return "user_nocache";
}

function model_commentsMetaData()
{
	return array(
		// pagination
...
function model_commentsJavascript($block_config)
{
	global $config;
...
function model_commentsAsync($block_config)
{
	global $config,$database_selectors;

	if ($_REQUEST['action']=='add_comment' && intval($_REQUEST['model_id'])>0)
	{
		...
		$result=sql("select * from $config[tables_prefix]models where model_id=$model_id");
		if (mr2rows($result)==0)
		{
			async_return_request_status(array(array('error_code'=>'invalid_model','block'=>'model_comments')));
		}
		$model_info=mr2array_single($result);
		...
		if (strpos($antispam_action, 'error') !== false)
		{
			...
			async_return_request_status(array(array('error_code'=>'spam','block'=>'model_comments')));
		}
		...
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'required','error_field_code'=>'comment_error_1','block'=>'model_comments');
		} elseif (intval($block_config['min_length'])>0 && strlen(trim($comment))<intval($block_config['min_length']))
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'minimum','error_details'=>array(intval($block_config['min_length'])),'error_field_code'=>'comment_error_2','block'=>'model_comments');
		}
		if ($use_captcha==1)
		{
			$recaptcha_data=null;
			...
			if (is_array($recaptcha_data) && $recaptcha_data['site_key'])
			{
				...
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'required','error_field_code'=>'code_error_5','block'=>'model_comments');
				} elseif (!validate_recaptcha($recaptcha_response, $recaptcha_data))
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'invalid','error_field_code'=>'code_error_6','block'=>'model_comments');
				}
			} else
			{
				if (strlen($code)==0)
		...
		if (!is_array($errors))
		{
			if ($user_id>0)
			{
				...
				if ($approved==0)
				{
					...
					$tokens_granted=0;
					if (strlen($comment)>=intval($memberzone_data['AWARDS_COMMENT_MODEL_CONDITION']))
					{
					...
					sql("update $config[tables_prefix]models set comments_count=(select count(*) from $config[tables_prefix]comments where object_id=$config[tables_prefix]models.model_id and object_type_id=4 and is_approved=1) where model_id=$model_id");

					if (function_exists('inc_block_version'))
					{
						inc_block_version('comments_info','model',$model_info['model_id'],$model_info['dir']);
						inc_block_version('models_info','model',$model_info['model_id'],$model_info['dir']);
					}
				}
				...
				if ($is_anonymous==0)
				{
					sql_pr("insert into $config[tables_prefix]users_events set event_type_id=14, user_id=?, model_id=?, comment_id=?, added_date=?",$user_id,$model_id,$item_id,$comment_date);
				}
				...
				async_return_request_status(array(array('error_code'=>'invalid_params','block'=>'model_comments')));
			}
		} else {
	...
	} elseif ($_REQUEST['action']=='vote_comment' && intval($_REQUEST['comment_id'])>0)
	{
		if (intval($_REQUEST['vote'])!=0)
		{
			...
			if (intval($comment_info['comment_id'])>0)
			{
				...
				{
					async_return_request_status(array(array('error_code'=>'ip_already_voted','error_field_code'=>'error_1','block'=>'model_comments')));
				} else {
					...
					if (intval($comment_info['object_type_id'])==4)
					{
						$model_id=intval($comment_info['object_id']);
						$model_info=mr2array_single(sql("select model_id, $database_selectors[generic_selector_dir] as dir from $config[tables_prefix]models where model_id=$model_id"));
						if ($model_info['model_id']>0 && function_exists('inc_block_version'))
						{
							inc_block_version('comments_info','model',$model_info['model_id'],$model_info['dir']);
							inc_block_version('models_info','model',$model_info['model_id'],$model_info['dir']);
						}
	...
	} elseif ($_REQUEST['action']=='edit_comment' && intval($_REQUEST['comment_id'])>0)
	{
		...
		if (!isset($block_config['allow_editing']))
		{
			async_return_request_status(array(array('error_code'=>'editing_not_allowed','block'=>'model_comments')));
		}
		if (intval($_SESSION['user_id'])>0)
		{
			$comment_id=intval($_REQUEST['comment_id']);
			...
			async_return_request_status(array(array('error_code'=>'not_logged_in','block'=>'model_comments')));
		}
	}
}
