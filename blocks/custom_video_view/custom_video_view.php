<?php
function custom_video_viewShow($block_config,$object_id)
{
	...
	$count = mr2number(sql_pr("select count(*) from ktvs_dvds where user_id=?",$data['user']['user_id']));
    if ($count>0)
    {
        $data['user']['dvds_count'] = $count;
        $data['user']['dvds'] = mr2array_single(sql_pr("select * from ktvs_dvds where user_id=?",$data['user']['user_id']));

		if ($_SESSION['user_id']>0)
		{
			$data['user']['dvds']['is_subscribed']=0;
			if (mr2number(sql_pr("select count(*) from $config[tables_prefix]users_subscriptions where user_id=? and subscribed_object_id=? and subscribed_type_id=5",$_SESSION['user_id'],$data['user']['dvds']['dvd_id']))>0)
			{
				$data['user']['dvds']['is_subscribed']=1;
			}
		}
	...
	if ($_SESSION['user_id']>0)
	{
		if ($_SESSION['user_id']<>$data['user_id'])
		{
			...
			if (custom_video_viewHasPremiumAccessByTokens(0,$data['user_id'],0))
			{
	...
	if ($data['dvd_id']>0)
	{
		...
		if (mr2rows($result)>0)
		{
			...
			if ($_SESSION['user_id']>0)
			{
				...
				if (custom_video_viewHasPremiumAccessByTokens(0,0,$data['dvd_id']))
				{
	...
	$video_log=custom_view_videoGetVideoLog($block_config);
	...
	$allowed_postfixes_for_view=custom_video_viewGetAllowedPostfixesForCurrentUser($data['video_id'],$data['user_id'],$data['dvd_id']);
	$allowed_postfixes_for_download=custom_video_viewGetAllowedPostfixesForDownload($data['video_id'],$data['user_id'],$data['dvd_id']);
	...
	foreach ($formats as $format_rec)
	{
		...
		if (in_array($format_rec['postfix'],$allowed_postfixes_for_download))
		{
			...
			if ($data['user_id']==$_SESSION['user_id'] || custom_video_viewHasPremiumAccessByTokens(0,$data['user_id'],$data['dvd_id']))
			{
	...
	if ($_SESSION['user_id']==$data['user_id'] || custom_video_viewHasPremiumAccessByTokens($data['video_id'],$data['user_id'],$data['dvd_id']))
	{
	...
	$flash_vars=custom_video_viewGetFlashVars($data,0);
	$flash_vars_embed=custom_video_viewGetFlashVars($data,1,$block_config['embed_player_profile_id']);
...
function custom_video_viewGetHash($block_config)
{
	global $config;
...
function custom_video_viewCacheControl($block_config)
{
	return "user_nocache";
}

function custom_video_viewMetaData()
{
	return array(
		// object context
...
function custom_video_viewJavascript($block_config)
{
	global $config;
...
function custom_video_viewPreProcess($block_config,$object_id)
{
	global $config, $database_selectors;

	custom_video_viewProcessRotatorParams($block_config);
...
function custom_video_viewAsync($block_config)
{
...
function custom_view_videoGetVideoLog($block_config)
{
	global $config;
...
function custom_video_viewGetFlashVars($video_data,$is_embed,$embed_player_profile_id='')
{
	...
	if ($is_embed==1)
	{
		...
		if ($_SESSION['status_id']==3 || $video_data['user_id']==$_SESSION['user_id'] || custom_video_viewHasPremiumAccessByTokens($video_data['video_id'],$video_data['user_id'],$video_data['dvd_id']))
		{
	...
	if (count($slots)>0)
	{
		$allowed_postfixes=custom_video_viewGetAllowedPostfixesForCurrentUser($video_data['video_id'],$video_data['user_id'],$video_data['dvd_id']);
		foreach ($slots as $k=>$slot)
		{
			if ($slot['type']=='redirect')
			{
				...
				} elseif ($player_data['format_redirect_url_source']==2)
				{
					...
					$slots[$k]['redirect_url']=custom_video_viewAddAffiliateParam("$config[project_url]/$pattern",$player_data);
			...
			foreach ($formats as $format_rec)
			{
				if ($format_rec['postfix']==$slot['type'])
				{
					...
					if ($video_data['user_id']==$_SESSION['user_id'] || custom_video_viewHasPremiumAccessByTokens(0,$video_data['user_id'],$video_data['dvd_id']))
					{
	...
	if ($player_data['logo_source']==0)
	{
		...
		$logo_file=custom_video_viewGetFromCS($video_data['content_source'],$player_data['logo_source']);
		if ($logo_file!='')
		{
	...
	if ($flash_vars['logo_src']!='' || $flash_vars['logo_text']!='')
	{
		...
		{
			$pattern=str_replace("%ID%",$video_data['video_id'],str_replace("%DIR%",$video_data['dir'],$website_ui_data['WEBSITE_LINK_PATTERN']));
			$flash_vars['logo_url']=custom_video_viewAddAffiliateParam("$config[project_url]/$pattern",$player_data);
		} elseif ($video_data['content_source_id']>0)
		{
	...
	if ($player_data['enable_video_click']==1)
	{
		...
		{
			$pattern=str_replace("%ID%",$video_data['video_id'],str_replace("%DIR%",$video_data['dir'],$website_ui_data['WEBSITE_LINK_PATTERN']));
			$flash_vars['video_click_url']=custom_video_viewAddAffiliateParam("$config[project_url]/$pattern",$player_data);
		} elseif ($video_data['content_source_id']>0)
		{
	...
	{
		$pattern=str_replace("%ID%",$video_data['video_id'],str_replace("%DIR%",$video_data['dir'],$website_ui_data['WEBSITE_LINK_PATTERN']));
		$flash_vars['mlogo_link']=custom_video_viewAddAffiliateParam("$config[project_url]/$pattern",$player_data);
	} elseif ($video_data['content_source_id']>0)
	{
	...
	if ($player_data['enable_pre_roll']==1)
	{
		...
		if ($player_data['pre_roll_file_source']==1)
		{
			...
			$pre_roll_file=custom_video_viewGetFromCS($video_data['content_source'],$player_data['pre_roll_file_source']-1);
			if ($pre_roll_file<>'')
			{
	...
	if ($player_data['enable_post_roll']==1)
	{
		...
		if ($player_data['post_roll_file_source']==1)
		{
			...
			$post_roll_file=custom_video_viewGetFromCS($video_data['content_source'],$player_data['post_roll_file_source']-1);
			if ($post_roll_file<>'')
			{
	...
	if ($player_data['enable_pause']==1)
	{
		...
		if ($player_data['pause_file_source']==1)
		{
			...
			$pause_file=custom_video_viewGetFromCS($video_data['content_source'],$player_data['pause_file_source']-1);
			if ($pause_file<>'')
			{
...
function custom_video_viewGetFromCS($cs_data,$file_num)
{
	global $config;
...
function custom_video_viewGetAllowedPostfixesForCurrentUser($video_id,$owner_id,$dvd_id)
{
	...
	foreach ($formats_videos as $format)
	{
		...
		{
			$result[]=$format['postfix'];
		} elseif ($format['access_level_id']==2 && ($_SESSION['status_id']==3 || $owner_id==$_SESSION['user_id'] || custom_video_viewHasPremiumAccessByTokens($video_id,$owner_id,$dvd_id)))
		{
			$result[]=$format['postfix'];
		}
	}
...
function custom_video_viewGetAllowedPostfixesForEveryone()
{
	global $formats_videos;
...
function custom_video_viewGetAllowedPostfixesForDownload($video_id,$owner_id,$dvd_id)
{
	global $formats_videos;
...
function custom_video_viewHasPremiumAccessByTokens($video_id,$owner_id,$dvd_id)
{
...
function custom_video_viewProcessRotatorParams($block_config)
{
	global $config;
...
function custom_video_viewAddAffiliateParam($url,$player_data)
{
