<?php
function video_commentsShow($block_config,$object_id)
{
	...
	$video_info=mr2array_single($result);
	...
	if (isset($block_config['var_from']))
	{
		...
		$total_count=mr2number(sql_pr("select count(*) from $config[tables_prefix]comments where object_id=$video_id and object_type_id=1 and is_approved=1 and added_date<=? $where",date("Y-m-d H:i:s")));
		...
		$data=mr2array(sql_pr("select $config[tables_prefix]comments.*, $config[tables_prefix]users.status_id, $config[tables_prefix]users.avatar, $config[tables_prefix]users.username, $config[tables_prefix]users.display_name, $config[tables_prefix]users.gender_id, $config[tables_prefix]users.birth_date, $config[tables_prefix]users.country_id, $config[tables_prefix]users.city from $config[tables_prefix]comments left join $config[tables_prefix]users on $config[tables_prefix]comments.user_id=$config[tables_prefix]users.user_id where $config[tables_prefix]comments.object_id=? and $config[tables_prefix]comments.object_type_id=1 and $config[tables_prefix]comments.is_approved=1 and $config[tables_prefix]comments.added_date<=? $where order by $config[tables_prefix]comments.$sort_by $limit",$video_id,date("Y-m-d H:i:s")));
	...
	if ($video_info['is_private']==2)
	{
		if (intval($memberzone_data['ENABLE_TOKENS_PREMIUM_VIDEO'])==1)
		{
			if (intval($video_info['tokens_required'])==0)
			{
				$video_info['tokens_required']=intval($memberzone_data['DEFAULT_TOKENS_PREMIUM_VIDEO']);
			}
		} else {
			$video_info['tokens_required']=0;
		}
	} else {
		if (intval($memberzone_data['ENABLE_TOKENS_STANDARD_VIDEO'])==1)
		{
			if (intval($video_info['tokens_required'])==0)
			{
				$video_info['tokens_required']=intval($memberzone_data['DEFAULT_TOKENS_STANDARD_VIDEO']);
			}
		} else {
			$video_info['tokens_required']=0;
		}
	...
	$video_info['is_purchased_video']=0;
	if ($_SESSION['user_id']==$video_info['user_id'] || video_commentsHasPremiumAccessByTokens($video_info['video_id'],$video_info['user_id'],$video_info['dvd_id']))
	{
		$video_info['is_purchased_video']=1;
	}

	$can_watch_video=1;
	if ($video_info['is_private']==0 && $_SESSION['user_id']<>$video_info['user_id'])
	{
		$can_watch_video=0;
		$access_option=intval($memberzone_data['PUBLIC_VIDEOS_ACCESS']);
		if (intval($video_info['access_level_id'])==1)
		{
			$access_option=0;
		} elseif (intval($video_info['access_level_id'])==2)
		{
			$access_option=1;
		} elseif (intval($video_info['access_level_id'])==3)
		{
			$access_option=2;
		}
		switch ($access_option)
		{
			...
			case 1:
				...
				break;
			case 2:
				if ($_SESSION['status_id']==3 || $video_info['is_purchased_video']==1)
				{
					$can_watch_video=1;
				}
				break;
	...
	if ($video_info['is_private']==1 && $_SESSION['user_id']<>$video_info['user_id'])
	{
		$can_watch_video=0;
		...
		switch ($access_option)
		{
			case 0:
				...
				break;
			case 1:
				if ($_SESSION['user_id']>0 && mr2number(sql_pr("select count(*) from $config[tables_prefix]friends where is_approved=1 and ((user_id=? and friend_id=?) or (friend_id=? and user_id=?))",$_SESSION['user_id'],$video_info['user_id'],$_SESSION['user_id'],$video_info['user_id']))>0)
				{
					$can_watch_video=1;
				}
				break;
	...
	if ($video_info['is_private']==2 && $_SESSION['user_id']<>$video_info['user_id'])
	{
		$can_watch_video=0;
	...
	$video_info['can_watch']=$can_watch_video;

	$smarty->assign("data",$data);
	$smarty->assign("can_watch_video",$can_watch_video);
	if ($can_watch_video==1)
	{
...
function video_commentsGetHash($block_config)
{
	...
	{
		$current_version=get_block_version('comments_info','video',$id,$dir);
	}
...
function video_commentsCacheControl($block_config)
{
	return "user_nocache";
}

function video_commentsHasPremiumAccessByTokens($video_id,$owner_id,$dvd_id)
{
...
function video_commentsMetaData()
{
	return array(
		// pagination
...
function video_commentsJavascript($block_config)
{
	global $config;
...
function video_commentsAsync($block_config)
{
	global $config,$database_selectors;

	if ($_REQUEST['action']=='add_comment' && intval($_REQUEST['video_id'])>0)
	{
		...
		if (mr2rows($result)==0)
		{
			async_return_request_status(array(array('error_code'=>'invalid_video','block'=>'video_comments')));
		}
		$video_info=mr2array_single($result);
		...
		if (strpos($antispam_action, 'error') !== false)
		{
			...
			async_return_request_status(array(array('error_code'=>'spam','block'=>'video_comments')));
		}
		...
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'required','error_field_code'=>'comment_error_1','block'=>'video_comments');
		} elseif (intval($block_config['min_length'])>0 && strlen(trim($comment))<intval($block_config['min_length']))
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'minimum','error_details'=>array(intval($block_config['min_length'])),'error_field_code'=>'comment_error_2','block'=>'video_comments');
		}
		if ($use_captcha==1)
		{
			$recaptcha_data=null;
			...
			if (is_array($recaptcha_data) && $recaptcha_data['site_key'])
			{
				...
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'required','error_field_code'=>'code_error_5','block'=>'video_comments');
				} elseif (!validate_recaptcha($recaptcha_response, $recaptcha_data))
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'invalid','error_field_code'=>'code_error_6','block'=>'video_comments');
				}
			} else
			{
				if (strlen($code)==0)
		...
		if (!is_array($errors))
		{
			if ($user_id>0)
			{
				...
				if ($approved==0)
				{
					...
					$memberzone_data=unserialize(@file_get_contents("$config[project_path]/admin/data/system/memberzone_params.dat"));
					$tokens_granted=0;
					if (strlen($comment)>=intval($memberzone_data['AWARDS_COMMENT_VIDEO_CONDITION']))
					{
					...
					sql("update $config[tables_prefix]videos set comments_count=(select count(*) from $config[tables_prefix]comments where object_id=$config[tables_prefix]videos.video_id and object_type_id=1 and is_approved=1) where video_id=$video_id");

					if (function_exists('inc_block_version'))
					{
						inc_block_version('comments_info','video',$video_info['video_id'],$video_info['dir']);
						inc_block_version('videos_info','video',$video_info['video_id'],$video_info['dir']);
					}
				}
				...
				if ($is_anonymous==0)
				{
					sql_pr("insert into $config[tables_prefix]users_events set event_type_id=4, user_id=?, video_id=?, comment_id=?, added_date=?",$user_id,$video_id,$item_id,$comment_date);
				}
				...
				async_return_request_status(array(array('error_code'=>'invalid_params','block'=>'video_comments')));
			}
		} else {
	...
	} elseif ($_REQUEST['action']=='vote_comment' && intval($_REQUEST['comment_id'])>0)
	{
		if (intval($_REQUEST['vote'])!=0)
		{
			...
			if (intval($comment_info['comment_id'])>0)
			{
				...
				{
					async_return_request_status(array(array('error_code'=>'ip_already_voted','error_field_code'=>'error_1','block'=>'video_comments')));
				} else {
					...
					if (intval($comment_info['object_type_id'])==1)
					{
						$video_id=intval($comment_info['object_id']);
						$video_info=mr2array_single(sql("select video_id, $database_selectors[generic_selector_dir] as dir from $config[tables_prefix]videos where video_id=$video_id"));
						if ($video_info['video_id']>0 && function_exists('inc_block_version'))
						{
							inc_block_version('comments_info','video',$video_info['video_id'],$video_info['dir']);
							inc_block_version('videos_info','video',$video_info['video_id'],$video_info['dir']);
						}
	...
	} elseif ($_REQUEST['action']=='edit_comment' && intval($_REQUEST['comment_id'])>0)
	{
		...
		if (!isset($block_config['allow_editing']))
		{
			async_return_request_status(array(array('error_code'=>'editing_not_allowed','block'=>'video_comments')));
		}
		if (intval($_SESSION['user_id'])>0)
		{
			$comment_id=intval($_REQUEST['comment_id']);
			...
			async_return_request_status(array(array('error_code'=>'not_logged_in','block'=>'video_comments')));
		}
	}
}
