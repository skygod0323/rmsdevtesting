<?php
function dvd_commentsShow($block_config,$object_id)
{
	...
	$dvd_info=mr2array_single($result);
	$dvd_id=intval($dvd_info['dvd_id']);
	...
	if (isset($block_config['var_from']))
	{
		...
		$total_count=mr2number(sql_pr("select count(*) from $config[tables_prefix]comments where object_id=$dvd_id and object_type_id=5 and is_approved=1 and added_date<=? $where",date("Y-m-d H:i:s")));
		...
		$data=mr2array(sql_pr("select $config[tables_prefix]comments.*, $config[tables_prefix]users.status_id, $config[tables_prefix]users.avatar, $config[tables_prefix]users.username, $config[tables_prefix]users.display_name, $config[tables_prefix]users.gender_id, $config[tables_prefix]users.birth_date, $config[tables_prefix]users.country_id, $config[tables_prefix]users.city from $config[tables_prefix]comments left join $config[tables_prefix]users on $config[tables_prefix]comments.user_id=$config[tables_prefix]users.user_id where $config[tables_prefix]comments.object_id=? and $config[tables_prefix]comments.object_type_id=5 and $config[tables_prefix]comments.is_approved=1 and $config[tables_prefix]comments.added_date<=? $where order by $config[tables_prefix]comments.$sort_by $limit",$dvd_id,date("Y-m-d H:i:s")));
	...
	$smarty->assign("dvd_info",$dvd_info);
...
function dvd_commentsGetHash($block_config)
{
	...
	{
		$current_version=get_block_version('comments_info','dvd',$id,$dir);
	}
...
function dvd_commentsCacheControl($block_config)
{
	return "user_nocache";
}

function dvd_commentsMetaData()
{
	return array(
		// pagination
...
function dvd_commentsJavascript($block_config)
{
	global $config;
...
function dvd_commentsAsync($block_config)
{
	global $config,$database_selectors;

	if ($_REQUEST['action']=='add_comment' && intval($_REQUEST['dvd_id'])>0)
	{
		...
		$result=sql("select * from $config[tables_prefix]dvds where dvd_id=$dvd_id");
		if (mr2rows($result)==0)
		{
			async_return_request_status(array(array('error_code'=>'invalid_dvd','block'=>'dvd_comments')));
		}
		$dvd_info=mr2array_single($result);
		...
		if (strpos($antispam_action, 'error') !== false)
		{
			...
			async_return_request_status(array(array('error_code'=>'spam','block'=>'dvd_comments')));
		}
		...
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'required','error_field_code'=>'comment_error_1','block'=>'dvd_comments');
		} elseif (intval($block_config['min_length'])>0 && strlen(trim($comment))<intval($block_config['min_length']))
		{
			$errors[]=array('error_field_name'=>'comment','error_code'=>'minimum','error_details'=>array(intval($block_config['min_length'])),'error_field_code'=>'comment_error_2','block'=>'dvd_comments');
		}
		if ($use_captcha==1)
		{
			$recaptcha_data=null;
			...
			if (is_array($recaptcha_data) && $recaptcha_data['site_key'])
			{
				...
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'required','error_field_code'=>'code_error_5','block'=>'dvd_comments');
				} elseif (!validate_recaptcha($recaptcha_response, $recaptcha_data))
				{
					$errors[]=array('error_field_name'=>'code','error_code'=>'invalid','error_field_code'=>'code_error_6','block'=>'dvd_comments');
				}
			} else
			{
				if (strlen($code)==0)
		...
		if (!is_array($errors))
		{
			if ($user_id>0)
			{
				...
				if ($approved==0)
				{
					...
					$tokens_granted=0;
					if (strlen($comment)>=intval($memberzone_data['AWARDS_COMMENT_DVD_CONDITION']))
					{
					...
					sql("update $config[tables_prefix]dvds set comments_count=(select count(*) from $config[tables_prefix]comments where object_id=$config[tables_prefix]dvds.dvd_id and object_type_id=5 and is_approved=1) where dvd_id=$dvd_id");

					if (function_exists('inc_block_version'))
					{
						inc_block_version('comments_info','dvd',$dvd_info['dvd_id'],$dvd_info['dir']);
						inc_block_version('dvds_info','dvd',$dvd_info['dvd_id'],$dvd_info['dir']);
					}
				}
				...
				if ($is_anonymous==0)
				{
					sql_pr("insert into $config[tables_prefix]users_events set event_type_id=16, user_id=?, dvd_id=?, comment_id=?, added_date=?",$user_id,$dvd_id,$item_id,$comment_date);
				}
				...
				async_return_request_status(array(array('error_code'=>'invalid_params','block'=>'dvd_comments')));
			}
		} else {
	...
	} elseif ($_REQUEST['action']=='vote_comment' && intval($_REQUEST['comment_id'])>0)
	{
		if (intval($_REQUEST['vote'])!=0)
		{
			...
			if (intval($comment_info['comment_id'])>0)
			{
				...
				{
					async_return_request_status(array(array('error_code'=>'ip_already_voted','error_field_code'=>'error_1','block'=>'dvd_comments')));
				} else {
					...
					if (intval($comment_info['object_type_id'])==5)
					{
						$dvd_id=intval($comment_info['object_id']);
						$dvd_info=mr2array_single(sql("select dvd_id, $database_selectors[generic_selector_dir] as dir from $config[tables_prefix]dvds where dvd_id=$dvd_id"));
						if ($dvd_info['dvd_id']>0 && function_exists('inc_block_version'))
						{
							inc_block_version('comments_info','dvd',$dvd_info['dvd_id'],$dvd_info['dir']);
							inc_block_version('dvds_info','dvd',$dvd_info['dvd_id'],$dvd_info['dir']);
						}
	...
	} elseif ($_REQUEST['action']=='edit_comment' && intval($_REQUEST['comment_id'])>0)
	{
		...
		if (!isset($block_config['allow_editing']))
		{
			async_return_request_status(array(array('error_code'=>'editing_not_allowed','block'=>'dvd_comments')));
		}
		if (intval($_SESSION['user_id'])>0)
		{
			$comment_id=intval($_REQUEST['comment_id']);
			...
			async_return_request_status(array(array('error_code'=>'not_logged_in','block'=>'dvd_comments')));
		}
	}
}
