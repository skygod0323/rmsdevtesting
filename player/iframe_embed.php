...
function replace_runtime_params($page)
{
	...
	if ($pos !== false)
	{
		...
		while ($pos !== false)
		{
			...
			{
				if (strlen(trim($_COOKIE["kt_vast_$profile_id"])) > 0)
				{
				...
				foreach ($profile_info['providers'] as $provider)
				{
					if (intval($provider['is_enabled']) == 1)
					{
						...
						{
							$ads_empty[] = $provider;
							$ads_empty_sorting[] = intval($provider['weight']);
						} else
						{
