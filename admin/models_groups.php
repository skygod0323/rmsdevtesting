...
$table_fields[] = array('id' => 'status_id',      'title' => $lang['categorization']['model_group_field_status'],      'is_default' => 0, 'type' => 'choice', 'values' => $list_status_values);
...
$table_fields[] = array('id' => 'models_amount',  'title' => $lang['categorization']['model_group_field_models'],      'is_default' => 1, 'show_in_sidebar' => 1, 'type' => 'number', 'link' => 'models.php?no_filter=true&se_model_group_id=%id%', 'link_id' => 'model_group_id', 'permission' => 'models|view', 'ifdisable_zero' => 1);
...
if ($_REQUEST['batch_action'] <> '' && !isset($_REQUEST['reorder']) && count($_REQUEST['row_select']) > 0)
{
	...
	if ($_REQUEST['batch_action'] == 'delete')
	{
		...
		foreach ($data as $k => $v)
		{
			if (is_file("$config[content_path_models]/groups/$v[$table_key_name]/$v[screenshot1]"))
			{
				@unlink("$config[content_path_models]/groups/$v[$table_key_name]/$v[screenshot1]");
			}
			if (is_file("$config[content_path_models]/groups/$v[$table_key_name]/$v[screenshot2]"))
			{
				@unlink("$config[content_path_models]/groups/$v[$table_key_name]/$v[screenshot2]");
			}
			if (is_dir("$config[content_path_models]/groups/$v[$table_key_name]"))
			{
				@rmdir("$config[content_path_models]/groups/$v[$table_key_name]");
			}
