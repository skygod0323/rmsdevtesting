...
if (isset($_REQUEST['full_list']))
{
	...
	$smarty->assign('data', mr2array(sql("select tag as id, tag as title, status_id from $config[tables_prefix]tags where $where_status order by $applied_sorting[value]")));
...
} else
{
	...
	if (strlen($_REQUEST['for']) > 1)
	{
		...
		$data = mr2array(sql("select tag, synonyms, status_id from $config[tables_prefix]tags where (tag like '%$q%' or synonyms like '%$q%') and $where_status order by $applied_sorting[value]"));
		foreach ($data as $item)
		{
			...
			if ($synonyms != '')
			{
				...
				echo "<value synonyms=\"$synonyms\" $disabled>$tag</value>\n";
			} else
			{
				echo "<value $disabled>$tag</value>\n";
			}
		}
		echo "</insight>";
	} else
