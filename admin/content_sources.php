...
$table_fields[] = array('id' => 'status_id',            'title' => $lang['categorization']['content_source_field_status'],      'is_default' => 0, 'type' => 'choice', 'values' => $list_status_values);
...
$table_fields[] = array('id' => 'content_source_group', 'title' => $lang['categorization']['content_source_field_group'],       'is_default' => 1, 'type' => 'refid', 'link' => 'content_sources_groups.php?action=change&item_id=%id%', 'link_id' => 'content_source_group_id', 'permission' => 'content_sources_groups|view');
...
$table_fields[] = array('id' => 'tags',                 'title' => $lang['categorization']['content_source_field_tags'],        'is_default' => 0, 'type' => 'list', 'link' => 'tags.php?action=change&item_id=%id%', 'permission' => 'tags|view');
$table_fields[] = array('id' => 'categories',           'title' => $lang['categorization']['content_source_field_categories'],  'is_default' => 0, 'type' => 'list', 'link' => 'categories.php?action=change&item_id=%id%', 'permission' => 'categories|view');
...
if (isset($_GET['reset_filter']) || isset($_GET['no_filter']))
{
	...
	$_SESSION['save'][$page_name]['se_content_source_group_id'] = '';
...
if (!isset($_GET['reset_filter']))
{
	...
	if (isset($_GET['se_content_source_group_id']))
	{
		$_SESSION['save'][$page_name]['se_content_source_group_id'] = intval($_GET['se_content_source_group_id']);
	}
...
if ($_SESSION['save'][$page_name]['se_content_source_group_id'] > 0)
{
	$where .= " and $table_name.content_source_group_id=" . intval($_SESSION['save'][$page_name]['se_content_source_group_id']);
	$table_filtered = 1;
}
...
if (in_array($_POST['action'], array('add_new_complete', 'change_complete')))
{
	...
	if (!is_array($errors))
	{
		...
		{
			$item_id = sql_insert("insert into $table_name set content_source_group_id=?, title=?, dir=?, description=?, status_id=?, screenshot1=?, screenshot2=?, url=?, rating=?, rating_amount=?,
				...
				intval($_POST['content_source_group_id']), $_POST['title'], $_POST['dir'], $_POST['description'], intval($_POST['status_id']), $_POST['screenshot1'], $_POST['screenshot2'], $_POST['url'], floatval($_POST['avg_rating']) * intval($_POST['rating_amount']), intval($_POST['rating_amount']),
		...
		} else
		{
			...
			sql_pr("update $table_name set content_source_group_id=?, title=?, dir=?, description=?, status_id=?, url=?, rating=?, rating_amount=?,custom1=?, custom2=?, custom3=?, custom4=?, custom5=?, custom6=?, custom7=?, custom8=?, custom9=?, custom10=? where $table_key_name=?",
...
if ($_REQUEST['batch_action'] <> '' && !isset($_REQUEST['reorder']) && count($_REQUEST['row_select']) > 0)
{
	...
	if ($_REQUEST['batch_action'] == 'delete')
	{
		...
		foreach ($data as $k => $v)
		{
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[screenshot1]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[screenshot1]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[screenshot2]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[screenshot2]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file1]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file1]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file2]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file2]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file3]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file3]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file4]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file4]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file5]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file5]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file6]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file6]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file7]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file7]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file8]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file8]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file9]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file9]");
			}
			if (is_file("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file10]"))
			{
				@unlink("$config[content_path_content_sources]/$v[content_source_id]/$v[custom_file10]");
			}
			if (is_dir("$config[content_path_content_sources]/$v[content_source_id]"))
			{
				@rmdir("$config[content_path_content_sources]/$v[content_source_id]");
			}
		...
		sql("delete from $config[tables_prefix]users_subscriptions where subscribed_object_id in ($row_select) and subscribed_type_id=3");
...
$smarty->assign('list_content_sources_groups', mr2array(sql("select content_source_group_id, title from $config[tables_prefix]content_sources_groups order by title asc")));
