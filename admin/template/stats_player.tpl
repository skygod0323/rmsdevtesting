	...
	<form action="{{$page_name}}" method="get" class="form_dgf">
		<div class="dgf">
			<table>
				<tr>
					...
					<td class="dgf_label">{{$lang.stats.player_filter_group_by}}:</td>
					<td class="dgf_control">
						<select name="se_group_by" class="dgf_switcher">
							<option value="date" {{if $smarty.session.save.$page_name.se_group_by=='date'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_group_by_date}}</option>
							<option value="referer" {{if $smarty.session.save.$page_name.se_group_by=='referer'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_group_by_referer}}</option>
							<option value="country" {{if $smarty.session.save.$page_name.se_group_by=='country'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_group_by_country}}</option>
							<option value="device" {{if $smarty.session.save.$page_name.se_group_by=='device'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_group_by_device}}</option>
							<option value="embed_profile" {{if $smarty.session.save.$page_name.se_group_by=='embed_profile'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_group_by_embed_profile}}</option>
						</select>
					</td>
					<td class="dgf_control">
		...
		<div class="dgf dgf_advanced_filters {{if $table_filtered==0}}hidden{{/if}}">
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_location!=''}}dgf_selected{{/if}}">{{$lang.stats.player_filter_location}}:</td>
					<td class="dgf_control">
						<select name="se_location">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="own" {{if $smarty.session.save.$page_name.se_location=='own'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_location_own}}</option>
							<option value="embed" {{if $smarty.session.save.$page_name.se_location=='embed'}}selected="selected"{{/if}}>{{$lang.stats.player_filter_location_embed}}</option>
						</select>
					</td>
				</tr>
			</table>
					...
					<td class="dgf_control">
						<div class="de_vis_sw_select">
							<select name="se_device">
								<option value="">{{$lang.common.dg_filter_option_all}}</option>
								{{foreach from=$list_device_values|smarty:nodefaults item="title" key="key"}}
									<option value="{{$key}}" {{if $smarty.session.save.$page_name.se_device!='' && $smarty.session.save.$page_name.se_device==$key}}selected="selected"{{/if}}>{{$title}}</option>
								{{/foreach}}
							</select>
						</div>
					</td>
			...
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_embed_profile!=''}}dgf_selected{{/if}}">{{$lang.stats.player_field_embed_profile}}:</td>
					<td class="dgf_control">
						<div class="de_vis_sw_select">
							<select name="se_embed_profile">
								<option value="">{{$lang.common.dg_filter_option_all}}</option>
								{{foreach from=$list_embed_profile_values|smarty:nodefaults item="title" key="key"}}
									<option value="{{$key}}" {{if $smarty.session.save.$page_name.se_embed_profile!='' && $smarty.session.save.$page_name.se_embed_profile==$key}}selected="selected"{{/if}}>{{$title}}</option>
								{{/foreach}}
							</select>
						</div>
					</td>
