...
{{if in_array('category_groups|edit_all',$smarty.session.permissions) || (in_array('category_groups|add',$smarty.session.permissions) && $smarty.get.action=='add_new')}}
...
<form action="{{$page_name}}" method="post">
	...
	<table class="de {{if $can_edit_all==0}}de_readonly{{/if}}">
		...
		<tr>
			<td class="de_header" colspan="3"><div><a href="{{$page_name}}">{{$lang.categorization.submenu_option_category_groups_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.categorization.category_group_add}}{{else}}{{$lang.categorization.category_group_edit|replace:"%1%":$smarty.post.title}}{{/if}}</div></td>
		</tr>
		...
		{{/if}}
		{{if $options.ENABLE_CATEGORY_GROUP_FIELD_1==1 || $options.ENABLE_CATEGORY_GROUP_FIELD_2==1 || $options.ENABLE_CATEGORY_GROUP_FIELD_3==1}}
			<tr>
				<td class="de_separator" colspan="3"><div>{{$lang.categorization.category_group_divider_general}}</div></td>
			</tr>
		{{/if}}
		<tr>
			<td class="de_label de_required">{{$lang.categorization.category_group_field_title}} (*):</td>
			<td class="de_control">
			...
			{{if is_array($sidebar_fields)}}
				...
				{{/if}}
				{{if $image_field!=''}}
					{{assign var="sidebar_image_url" value="`$config.content_url_categories`/groups/`$smarty.post.category_group_id`/`$smarty.post.$image_field`"}}
				{{/if}}
		...
		{{if $smarty.get.action=='change'}}
			<tr>
				<td class="de_label">{{$lang.categorization.category_group_field_directory}}:</td>
				<td class="de_control">
					<input type="text" name="dir" maxlength="255" class="dyn_full_size" value="{{$smarty.post.dir}}"/>
					{{if $smarty.session.userdata.is_expert_mode==0}}
						<br/><span class="de_hint">{{$lang.categorization.category_group_field_directory_hint|replace:"%1%":$lang.categorization.category_group_field_title}}</span>
					{{/if}}
				</td>
			</tr>
		{{/if}}
		<tr>
			<td class="de_label">{{$lang.categorization.category_group_field_external_id}}:</td>
			<td class="de_control">
				<input type="text" name="external_id" maxlength="100" class="dyn_full_size" value="{{$smarty.post.external_id}}"/>
				{{if $smarty.session.userdata.is_expert_mode==0}}
					<br/><span class="de_hint">{{$lang.categorization.category_group_field_external_id_hint}}</span>
				{{/if}}
			</td>
		</tr>
		<tr>
			<td class="de_label">{{$lang.categorization.category_group_field_description}}:</td>
			<td class="de_control">
				<div class="de_str_len">
		...
		<tr>
			<td class="de_label">{{$lang.categorization.category_group_field_status}}:</td>
			<td class="de_control">
				<div class="de_lv_pair"><input type="checkbox" name="status_id" value="1" {{if $smarty.post.status_id=='1'}}checked="checked"{{/if}}/><label>{{$lang.categorization.category_group_field_status_active}}</label></div>
				{{if $smarty.session.userdata.is_expert_mode==0}}
					<br/><span class="de_hint">{{$lang.categorization.category_group_field_status_hint}}</span>
				{{/if}}
			</td>
		</tr>
		<tr>
			<td class="de_label">{{$lang.categorization.category_group_field_screenshot1}}:</td>
			<td class="de_control">
				<div class="de_fu">
					<div class="js_params">
						<span class="js_param">title={{$lang.categorization.category_group_field_screenshot1}}</span>
						...
						{{if $smarty.get.action=='change' && $smarty.post.screenshot1!='' && in_array(end(explode(".",$smarty.post.screenshot1)),explode(",",$config.image_allowed_ext))}}
							<span class="js_param">preview_url={{$config.content_url_categories}}/groups/{{$smarty.post.category_group_id}}/{{$smarty.post.screenshot1}}</span>
						{{/if}}
					</div>
				...
				{{if $smarty.session.userdata.is_expert_mode==0}}
					<br/><span class="de_hint">{{$lang.categorization.category_group_field_screenshot1_hint}} (<a href="options.php?page=general_settings">{{$options.CATEGORY_AVATAR_SIZE}}</a>)</span>
				{{/if}}
			</td>
		</tr>
		{{if $options.CATEGORY_AVATAR_OPTION>0}}
			<tr>
				<td class="de_label">{{$lang.categorization.category_group_field_screenshot2}}:</td>
				<td class="de_control">
					<div class="de_fu">
						<div class="js_params">
							<span class="js_param">title={{$lang.categorization.category_group_field_screenshot2}}</span>
							<span class="js_param">accept={{$config.image_allowed_ext}}</span>
							{{if $smarty.get.action=='change' && $smarty.post.screenshot2!='' && in_array(end(explode(".",$smarty.post.screenshot2)),explode(",",$config.image_allowed_ext))}}
								<span class="js_param">preview_url={{$config.content_url_categories}}/groups/{{$smarty.post.category_group_id}}/{{$smarty.post.screenshot2}}</span>
							{{/if}}
						</div>
					...
					{{if $smarty.session.userdata.is_expert_mode==0}}
						<br/>
						<span class="de_hint">
							{{$lang.categorization.category_group_field_screenshot2_hint}} (<a href="options.php?page=general_settings">{{$options.CATEGORY_AVATAR_2_SIZE}}</a>){{if $options.CATEGORY_AVATAR_OPTION==1}}; {{$lang.categorization.category_group_field_screenshot2_hint2|replace:"%1%":$lang.categorization.category_group_field_screenshot1}}{{/if}}
						</span>
					{{/if}}
				</td>
			</tr>
		...
		{{if $options.ENABLE_CATEGORY_GROUP_FIELD_1==1 || $options.ENABLE_CATEGORY_GROUP_FIELD_2==1 || $options.ENABLE_CATEGORY_GROUP_FIELD_3==1}}
			<tr>
				<td class="de_separator" colspan="3"><div>{{$lang.categorization.category_group_divider_customization}}</div></td>
			</tr>
			{{if $options.ENABLE_CATEGORY_GROUP_FIELD_1==1}}
				<tr>
					<td class="de_label">{{$options.CATEGORY_GROUP_FIELD_1_NAME}}:</td>
			...
			{{/if}}
			{{if $options.ENABLE_CATEGORY_GROUP_FIELD_2==1}}
				<tr>
					<td class="de_label">{{$options.CATEGORY_GROUP_FIELD_2_NAME}}:</td>
					<td class="de_control" colspan="2">
						<div class="de_str_len">
			...
			{{/if}}
			{{if $options.ENABLE_CATEGORY_GROUP_FIELD_3==1}}
				<tr>
					<td class="de_label">{{$options.CATEGORY_GROUP_FIELD_3_NAME}}:</td>
					<td class="de_control" colspan="2">
						<div class="de_str_len">
...
{{if in_array('category_groups|delete',$smarty.session.permissions)}}
	{{assign var=can_delete value=1}}
{{else}}
	{{assign var=can_delete value=0}}
{{/if}}
{{if in_array('category_groups|edit_all',$smarty.session.permissions)}}
	...
	<form action="{{$page_name}}" method="get" class="form_dgf">
		...
		<div class="dgf dgf_advanced_filters {{if $table_filtered==0}}hidden{{/if}}">
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_status_id!=''}}dgf_selected{{/if}}">{{$lang.categorization.category_group_field_status}}:</td>
					<td class="dgf_control">
						<select name="se_status_id">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="1" {{if $smarty.session.save.$page_name.se_status_id=='1'}}selected="selected"{{/if}}>{{$lang.categorization.category_group_field_status_active}}</option>
							<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected="selected"{{/if}}>{{$lang.categorization.category_group_field_status_disabled}}</option>
						</select>
					</td>
				</tr>
			</table>
				...
				<tr>
					...
					<td class="dgf_control">
						<select name="se_field">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="empty/description" {{if $smarty.session.save.$page_name.se_field=="empty/description"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.categorization.category_group_field_description}}</option>
							<option value="empty/screenshot1" {{if $smarty.session.save.$page_name.se_field=="empty/screenshot1"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.categorization.category_group_field_screenshot1}}</option>
							<option value="empty/screenshot2" {{if $smarty.session.save.$page_name.se_field=="empty/screenshot2"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.categorization.category_group_field_screenshot2}}</option>
							{{section name="data" start="1" loop="11"}}
								{{assign var="custom_field_id" value="custom`$smarty.section.data.index`"}}
								{{assign var="custom_field_name_id" value="CATEGORY_GROUP_FIELD_`$smarty.section.data.index`_NAME"}}
								{{assign var="custom_field_enable_id" value="ENABLE_CATEGORY_GROUP_FIELD_`$smarty.section.data.index`"}}
								...
								{{/if}}
							{{/section}}
							<option value="filled/description" {{if $smarty.session.save.$page_name.se_field=="filled/description"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.categorization.category_group_field_description}}</option>
							<option value="filled/screenshot1" {{if $smarty.session.save.$page_name.se_field=="filled/screenshot1"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.categorization.category_group_field_screenshot1}}</option>
							<option value="filled/screenshot2" {{if $smarty.session.save.$page_name.se_field=="filled/screenshot2"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.categorization.category_group_field_screenshot2}}</option>
							{{section name="data" start="1" loop="11"}}
								{{assign var="custom_field_id" value="custom`$smarty.section.data.index`"}}
								{{assign var="custom_field_name_id" value="CATEGORY_GROUP_FIELD_`$smarty.section.data.index`_NAME"}}
								{{assign var="custom_field_enable_id" value="ENABLE_CATEGORY_GROUP_FIELD_`$smarty.section.data.index`"}}
				...
				<tr>
					...
					<td class="dgf_control">
						<select name="se_usage">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="used/categories" {{if $smarty.session.save.$page_name.se_usage=="used/categories"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_usage_categories}}</option>
							<option value="notused/categories" {{if $smarty.session.save.$page_name.se_usage=="notused/categories"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_usage_no_categories}}</option>
						</select>
					</td>
				</tr>
			</table>
			...
			{{if $can_invoke_additional==1}}
				<ul class="dg_additional_menu_template">
					...
					{{if $can_see_audit_log==1}}
						<li class="js_params">
							<span class="js_param">href=log_audit.php?no_filter=true&amp;se_object_type_id=7&amp;se_object_id=${id}</span>
