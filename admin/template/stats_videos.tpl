	...
	<form action="{{$page_name}}" method="get" class="form_dgf">
		<div class="dgf">
			<table>
				<tr>
					...
					<td class="dgf_label">{{$lang.stats.videos_filter_group_by}}:</td>
					<td class="dgf_control">
						<select name="se_group_by" class="dgf_switcher">
							<option value="date" {{if $smarty.session.save.$page_name.se_group_by=='date'}}selected="selected"{{/if}}>{{$lang.stats.videos_filter_group_by_date}}</option>
							<option value="video" {{if $smarty.session.save.$page_name.se_group_by=='video'}}selected="selected"{{/if}}>{{$lang.stats.videos_filter_group_by_video}}</option>
						</select>
					</td>
					<td class="dgf_control">
		...
		<div class="dgf dgf_advanced_filters {{if $table_filtered==0}}hidden{{/if}}">
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_id!=''}}dgf_selected{{/if}}">{{$lang.stats.videos_field_id}}:</td>
					<td class="dgf_control"><input type="text" name="se_id" size="10" value="{{$smarty.session.save.$page_name.se_id}}"/></td>
				</tr>
			 </table>
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_storage_group_id>0}}dgf_selected{{/if}}">{{$lang.stats.videos_field_storage_group}}:</td>
					<td class="dgf_control">
			...
			<table>
				...
				{{foreach name=data item=item from=$data|smarty:nodefaults}}
					<tr class="dg_data{{if $smarty.foreach.data.iteration % 2==0}} dg_even{{/if}}">
						<td class="dg_selector"><input type="checkbox" name="row_select[]" value="{{$item.video_id}}" {{if $can_invoke_batch==0}}disabled="disabled"{{/if}}/></td>
					...
					{{if $can_invoke_batch==1}}
						...
						<td class="dgb_control">
							<select name="batch_action">
								...
								{{if $can_edit_all==1}}
									<option value="mass_select">{{$lang.stats.videos_batch_action_mass_select}}</option>
								{{/if}}
							</select>
						</td>
						<td class="dgb_control">
