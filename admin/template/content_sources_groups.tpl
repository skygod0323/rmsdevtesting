...
<form action="{{$page_name}}" method="post">
	...
	<table class="de {{if $can_edit_all==0}}de_readonly{{/if}}">
		...
		<tr>
			<td class="de_header" colspan="3"><div><a href="{{$page_name}}">{{$lang.categorization.submenu_option_content_source_groups_list}}</a> / {{if $smarty.get.action=='add_new'}}{{$lang.categorization.content_source_group_add}}{{else}}{{$lang.categorization.content_source_group_edit|replace:"%1%":$smarty.post.title}}{{/if}}</div></td>
		</tr>
		...
		{{if $options.ENABLE_CS_GROUP_FIELD_1==1 || $options.ENABLE_CS_GROUP_FIELD_2==1 || $options.ENABLE_CS_GROUP_FIELD_3==1 || $options.ENABLE_CS_GROUP_FIELD_4==1 || $options.ENABLE_CS_GROUP_FIELD_5==1}}
			<tr>
				<td class="de_separator" colspan="3"><div>{{$lang.categorization.content_source_group_divider_general}}</div></td>
			</tr>
		{{/if}}
		<tr>
		...
		{{if $smarty.get.action=='change'}}
			<tr>
				...
				<td class="de_control">
					...
					{{if $smarty.session.userdata.is_expert_mode==0}}
						<br/><span class="de_hint">{{$lang.categorization.content_source_group_field_directory_hint|replace:"%1%":$lang.categorization.content_source_group_field_title}}</span>
					{{/if}}
				</td>
			</tr>
		{{/if}}
		<tr>
			...
			<td class="de_control">
				...
				{{if $smarty.session.userdata.is_expert_mode==0}}
					<br/><span class="de_hint">{{$lang.categorization.content_source_group_field_external_id_hint}}</span>
				{{/if}}
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="de_control">
				<div class="de_lv_pair"><input type="checkbox" name="status_id" value="1" {{if $smarty.post.status_id=='1'}}checked="checked"{{/if}}/><label>{{$lang.categorization.content_source_group_field_status_active}}</label></div>
				{{if $smarty.session.userdata.is_expert_mode==0}}
					<br/><span class="de_hint">{{$lang.categorization.content_source_group_field_status_hint}}</span>
				{{/if}}
			</td>
		</tr>
		{{if $options.ENABLE_CS_GROUP_FIELD_1==1 || $options.ENABLE_CS_GROUP_FIELD_2==1 || $options.ENABLE_CS_GROUP_FIELD_3==1 || $options.ENABLE_CS_GROUP_FIELD_4==1 || $options.ENABLE_CS_GROUP_FIELD_5==1}}
			<tr>
				<td class="de_separator" colspan="3"><div>{{$lang.categorization.content_source_group_divider_customization}}</div></td>
			</tr>
	...
	<form action="{{$page_name}}" method="get" class="form_dgf">
		...
		<div class="dgf dgf_advanced_filters {{if $table_filtered==0}}hidden{{/if}}">
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_status_id!=''}}dgf_selected{{/if}}">{{$lang.categorization.content_source_group_field_status}}:</td>
					<td class="dgf_control">
						<select name="se_status_id">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="1" {{if $smarty.session.save.$page_name.se_status_id=='1'}}selected="selected"{{/if}}>{{$lang.categorization.content_source_group_field_status_active}}</option>
							<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected="selected"{{/if}}>{{$lang.categorization.content_source_group_field_status_disabled}}</option>
						</select>
					</td>
				</tr>
			</table>
				...
				<tr>
					...
					<td class="dgf_control">
						<select name="se_field">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="empty/description" {{if $smarty.session.save.$page_name.se_field=="empty/description"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_empty_field|replace:"%1%":$lang.categorization.content_source_group_field_description}}</option>
							{{section name="data" start="1" loop="11"}}
								...
								{{/if}}
							{{/section}}
							<option value="filled/description" {{if $smarty.session.save.$page_name.se_field=="filled/description"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_option_non_empty_field|replace:"%1%":$lang.categorization.content_source_group_field_description}}</option>
							{{section name="data" start="1" loop="11"}}
								{{assign var="custom_field_id" value="custom`$smarty.section.data.index`"}}
								{{assign var="custom_field_name_id" value="CS_GROUP_FIELD_`$smarty.section.data.index`_NAME"}}
								{{assign var="custom_field_enable_id" value="ENABLE_CS_GROUP_FIELD_`$smarty.section.data.index`"}}
				...
				<tr>
					...
					<td class="dgf_control">
						<select name="se_usage">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="used/content_sources" {{if $smarty.session.save.$page_name.se_usage=="used/content_sources"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_usage_content_sources}}</option>
							<option value="notused/content_sources" {{if $smarty.session.save.$page_name.se_usage=="notused/content_sources"}}selected="selected"{{/if}}>{{$lang.common.dg_filter_usage_no_content_sources}}</option>
						</select>
					</td>
				</tr>
			</table>
