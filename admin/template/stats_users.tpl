	...
	<form action="{{$page_name}}" method="get" class="form_dgf">
		...
		<div class="dgf dgf_advanced_filters {{if $table_filtered==0}}hidden{{/if}}">
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_user!=''}}dgf_selected{{/if}}">{{$lang.stats.users_field_user}}:</td>
					<td class="dgf_control">
						<div class="insight">
							<div class="js_params">
			...
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_status_id!=''}}dgf_selected{{/if}}">{{$lang.stats.users_field_status}}:</td>
					<td class="dgf_control">
						<select name="se_status_id">
							<option value="">{{$lang.common.dg_filter_option_all}}</option>
							<option value="0" {{if $smarty.session.save.$page_name.se_status_id=='0'}}selected="selected"{{/if}}>{{$lang.stats.users_field_status_disabled}}</option>
							<option value="2" {{if $smarty.session.save.$page_name.se_status_id=='2'}}selected="selected"{{/if}}>{{$lang.stats.users_field_status_active}}</option>
							<option value="3" {{if $smarty.session.save.$page_name.se_status_id=='3'}}selected="selected"{{/if}}>{{$lang.stats.users_field_status_premium}}</option>
							<option value="6" {{if $smarty.session.save.$page_name.se_status_id=='6'}}selected="selected"{{/if}}>{{$lang.stats.users_field_status_webmaster}}</option>
						</select>
					</td>
				</tr>
			</table>
			<table class="dgf_filter">
				<tr>
					<td class="dgf_label {{if $smarty.session.save.$page_name.se_reseller_code!=''}}dgf_selected{{/if}}">{{$lang.stats.users_field_reseller_code}}:</td>
