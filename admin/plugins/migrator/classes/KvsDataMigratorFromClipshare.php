...
class KvsDataMigratorFromClipshare extends KvsDataMigrator
{
	public const OPT_UPLOADS_FOLDER_NAME = "clipshare_uploads_folder_name";
	public const OPT_VIDEOS_BASE_URL = "clipshare_videos_base_url";
	...
	{
		return array(self::OPT_UPLOADS_FOLDER_NAME, self::OPT_VIDEOS_BASE_URL);
	}
	...
	protected function build_users_migration_params(): ?KvsDataMigratorMigrationParams
	{
		...
		$base_url = "$this->old_url/$uploads_folder_name/photo";
		$base_path = "$this->old_path/$uploads_folder_name/photo";
	...
	protected function build_videos_migration_params(): ?KvsDataMigratorMigrationParams
	{
		...
		$uploads_folder_name = $this->get_option_value(self::OPT_UPLOADS_FOLDER_NAME);
		$images_base_url = "$this->old_url/$uploads_folder_name/thumbs";

		$selector = "VID AS video_id, UID AS user_id, title, description, keyword AS tags, channel AS categories, duration, CASE WHEN type='private' THEN 1 ELSE 0 END AS is_private, addtime AS added_date, addtime AS post_date, vkey AS dir, viewnumber AS video_viewed, viewtime AS last_time_view_date, rate/2*ratedby AS rating, greatest(ratedby,1) AS rating_amount, CASE WHEN active='0' THEN 0 ELSE 1 END AS status_id, embed_code AS embed, concat('$images_base_url/1_', VID, '.jpg') AS screen_url, record_date AS custom1, location AS custom2, country AS custom3, CASE WHEN embed_code='' THEN concat('$videos_base_url/', flvdoname) ELSE '' END AS file_download_url";
