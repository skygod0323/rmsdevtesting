...
RewriteRule ^videos$ /latest-updates/ [R=301,L]
RewriteRule ^albums$ /albums/ [R=301,L]
RewriteRule ^blogs$ / [R=301,L]
RewriteRule ^categories$ /categories/ [R=301,L]
RewriteRule ^community$ /members/ [R=301,L]
RewriteRule ^signup$ / [R=301,L]
RewriteRule ^invite$ / [R=301,L]
RewriteRule ^notices$ / [R=301,L]
RewriteRule ^feedback$ / [R=301,L]
RewriteRule ^static/terms$ /terms/ [R=301,L]
RewriteRule ^static/privacy$ /terms/ [R=301,L]
RewriteRule ^static/dmca$ /dmca/ [R=301,L]
RewriteRule ^static/_2257$ /2257/ [R=301,L]
...
RewriteCond %{QUERY_STRING} ^search_query=([^&]+) [NC]
RewriteRule ^search/users$ /search/%1/ [R=301,L]

RewriteRule ^videos/([^/]+)$ /categories/$1/ [R=301,L]
RewriteRule ^albums/([^/]+)$ /albums/categories/$1/ [R=301,L]
RewriteRule ^photo/([0-9]+)/$ /albums/ [R=301,L]

RewriteRule ^user/([^/]+)$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/ [L,QSA]
RewriteRule ^user/([^/]+)/blog$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/ [L,QSA]
RewriteRule ^user/([^/]+)/wall$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/ [L,QSA]
RewriteRule ^user/([^/]+)/subscribers$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/ [L,QSA]
RewriteRule ^user/([^/]+)/subscriptions$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/ [L,QSA]
RewriteRule ^user/([^/]+)/playlist$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/favourites/videos/ [L,QSA]
RewriteRule ^user/([^/]+)/favorite/videos$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/favourites/videos/ [L,QSA]
RewriteRule ^user/([^/]+)/favorite/albums$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/favourites/albums/ [L,QSA]
RewriteRule ^user/([^/]+)/videos$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/videos/ [L,QSA]
RewriteRule ^user/([^/]+)/albums$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/albums/ [L,QSA]
RewriteRule ^user/([^/]+)/friends$ /kvs_redirect_username.php?username=$1&pattern=/members/MEMBER_ID/friends/ [L,QSA]

 */

class KvsDataMigratorFromAvs extends KvsDataMigrator
{
	public const OPT_USERS_BASE_URL = "avs_users_base_url"; // http://domain.com/media/users
	public const OPT_CATEGORIES_BASE_URL = "avs_categories_base_url"; // http://domain.com/media/categories/video
	public const OPT_CHANNELS_FRONT_BASE_URL = "avs_channels_front_base_url";
	public const OPT_CHANNELS_BACK_BASE_URL = "avs_channels_back_base_url";
	public const OPT_VIDEOS_BASE_URL = "avs_videos_base_url"; // http://domain.com/media/videos
	public const OPT_PHOTOS_BASE_URL = "avs_photos_base_url"; // http://domain.com/media/photos
	public const OPT_SCREEN_BASE_URL = "avs_screen_base_url"; // http://domain.com/media/videos/tmb
	public const OPT_AVS_VERSION = "avs_version"; // 4 or 6
	...
	{
		return new KvsDataMigratorDataToMigrate(true, true, false, false, true, true, true, true, true, true, true, true, true, true, false);
	}
	...
	{
		return array(self::OPT_AVS_VERSION, self::OPT_USERS_BASE_URL, self::OPT_CATEGORIES_BASE_URL, self::OPT_CHANNELS_FRONT_BASE_URL, self::OPT_CHANNELS_BACK_BASE_URL, self::OPT_VIDEOS_BASE_URL, self::OPT_PHOTOS_BASE_URL, self::OPT_SCREEN_BASE_URL);
	}
	...
	protected function build_progress_queries(): array
	{
		...
		if ($this->data_to_migrate->is_albums())
		{
			if ($this->get_test_mode() > 0)
			{
				$queries[] = "SELECT {$this->get_test_mode()} FROM albums LIMIT 1";
			} else
			{
				$queries[] = "SELECT count(*) FROM albums";
			}
		}
		if ($this->data_to_migrate->is_comments())
		{
		...
		if ($this->data_to_migrate->is_subscriptions())
		{
			...
			if ($this->is_v6())
			{
				$queries[] = "SELECT count(*) FROM channel_subscribe";
			}
		}
	...
	protected function build_categories_migration_params(): ?KvsDataMigratorMigrationParams
	{
		...
		if ($this->is_v6())
		{
			$selector = "CHID AS category_id, name AS title, description, slug AS dir, now() AS added_date";
		} else
		{
			$selector = "CHID AS category_id, name AS title, slug AS dir, now() AS added_date";
		}
		if (trim($categories_base_url) != "")
		{
			$selector .= ", concat(CHID, '.jpg') AS screenshot1, concat('$categories_base_url/', CHID, '.jpg') AS screenshot1_url";
		}
		return new KvsDataMigratorMigrationParams("category_id", "SELECT * FROM (SELECT $selector FROM channel) X", self::OBJECT_TYPE_CATEGORY);
	}
	...
	protected function build_dvds_migration_params(): ?KvsDataMigratorMigrationParams
	{
		if ($this->is_v6())
		{
			$channels_front_base_url = $this->get_option_value(self::OPT_CHANNELS_FRONT_BASE_URL);
			$channels_back_base_url = $this->get_option_value(self::OPT_CHANNELS_BACK_BASE_URL);

			$selector = "channel_id AS dvd_id, user_id, name AS title, slug AS dir, description, url AS custom1, now() AS added_date";
			if (trim($channels_front_base_url) != "")
			{
				$selector .= ", 'cover1.jpg' AS cover1_front, concat('$channels_front_base_url/', channel_id, '.jpg') AS cover1_front_url";
				$selector .= ", 'cover2.jpg' AS cover2_front, concat('$channels_front_base_url/', channel_id, 's.jpg') AS cover2_front_url";
			}
			if (trim($channels_back_base_url) != "")
			{
				$selector .= ", 'avatar1.jpg' AS cover1_back, concat('$channels_back_base_url/', channel_id, '.jpg') AS cover1_back_url";
				$selector .= ", 'avatar2.jpg' AS cover2_back, concat('$channels_back_base_url/', channel_id, 's.jpg') AS cover2_back_url";
			}
			return new KvsDataMigratorMigrationParams("dvd_id", "SELECT * FROM (SELECT $selector FROM channels) X", self::OBJECT_TYPE_DVD);
		}
		return null;
	}
	...
	protected function build_subscriptions_migration_params(): ?KvsDataMigratorMigrationParams
	{
		...
		if ($this->is_v6())
		{
			$selector .= " UNION ALL SELECT SUID AS user_id, UID AS subscribed_object_id, 5 AS subscribed_type_id, subscribe_date AS added_date FROM channel_subscribe";
		}
	 ...
	 * @return bool
	 */
	private function is_v6(): bool
	{
		return intval($this->get_option_value(self::OPT_AVS_VERSION)) >= 6;
	}
}