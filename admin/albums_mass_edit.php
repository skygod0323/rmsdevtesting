...
if ($_POST['action']=='change_complete')
{
	...
	$album_format_recreate_ids=$_POST['album_format_recreate_ids'];
	if ($_POST['tags_add']<>'')
	{
	...
	if (intval($_POST['post_date_option'])==0)
	{
		...
		{
			$errors[]=get_aa_error('invalid_date_range',$lang['albums']['mass_edit_albums_field_post_date']);
		...
		{
			if (!validate_field('empty_int_ext',$_POST['relative_post_date_from'],$lang['albums']['mass_edit_albums_field_post_date']))
			{
		...
		{
			if (!validate_field('empty_int_ext',$_POST['relative_post_date_to'],$lang['albums']['mass_edit_albums_field_post_date']))
			{
				$has_relative_post_date_error=1;
			}
		}
		if ($has_relative_post_date_error==0)
		{
			...
			{
				$errors[]=get_aa_error('invalid_int_range',$lang['albums']['mass_edit_albums_field_post_date']);
				$has_relative_post_date_error=1;
			}
		}
	}
	if (intval($_POST['post_time_change'])==1)
	{
		...
		{
			$errors[]=get_aa_error('invalid_time_range',$lang['albums']['mass_edit_albums_field_post_time']);
		}
	}
	...
	if ($_POST['visits_min']<>'' || $_POST['visits_max']<>'')
	{
		...
		{
			if (!validate_field('empty_int',$_POST['visits_min'],$lang['albums']['mass_edit_albums_field_visits']))
			{
				$has_visits_error=1;
			}
		}
		...
		{
			if (!validate_field('empty_int',$_POST['visits_max'],$lang['albums']['mass_edit_albums_field_visits']))
			{
				$has_visits_error=1;
			}
		}
		if ($has_visits_error==0)
		{
			...
			{
				$errors[]=get_aa_error('invalid_int_range',$lang['albums']['mass_edit_albums_field_visits']);
				$has_visits_error=1;
			}
		}
	}
	...
	if (!is_array($errors))
	{
		...
		if (@count($album_format_recreate_ids)>0)
		{
			$data['album_format_recreate_ids']=$album_format_recreate_ids;
			$needs_editing=1;
		}
...
$albums_count=mr2number(sql("select count(*) from $table_name where status_id in (0,1) and (album_id in ($ids_str) $all_str) $where"));
