...
if ($options['VIDEOS_IMPORT_PRESETS']<>'')
{
	$VIDEOS_IMPORT_PRESETS=@unserialize($options['VIDEOS_IMPORT_PRESETS']);
}
...
if ($_POST['preset_name']<>'')
{
	...
	$VIDEOS_IMPORT_PRESETS[$name]=$temp_data;

	if ($temp_data['is_default_preset']==1)
	{
		foreach ($VIDEOS_IMPORT_PRESETS as $k=>$preset)
		{
			if ($k<>$name && $preset['is_default_preset']==1)
			{
				$VIDEOS_IMPORT_PRESETS[$k]['is_default_preset']=0;
			}
		}
	}

	sql_pr("update $config[tables_prefix]options set value=? where variable='VIDEOS_IMPORT_PRESETS'",serialize($VIDEOS_IMPORT_PRESETS));
}
if ($_GET['action']!='back_import' && !isset($_GET['preset_id']) && count($_POST)==0 && is_array($VIDEOS_IMPORT_PRESETS))
{
	foreach ($VIDEOS_IMPORT_PRESETS as $k=>$preset)
	{
...
{
	unset($VIDEOS_IMPORT_PRESETS[$_POST['preset_id']]);
	sql_pr("update $config[tables_prefix]options set value=? where variable='VIDEOS_IMPORT_PRESETS'",serialize($VIDEOS_IMPORT_PRESETS));
...
{
	$_POST=$VIDEOS_IMPORT_PRESETS[$_GET['preset_id']];
}

$list_formats_videos=mr2array(sql("select * from $config[tables_prefix]formats_videos where status_id in (1,2) order by title"));
...
if ($_POST['action']=='start_import')
{
	...
	{
		validate_field('empty',"",$lang['videos']['import_field_data_text']);
	}
	...
	$is_field_error=0;
	if ($is_error) {$errors[]=get_aa_error('import_fields_required',$lang['videos']['import_divider_fields']);$is_field_error=1;}

	if ($is_field_error==0 && count($import_fiels_list)<>count(array_unique($import_fiels_list)))
	{
		$errors[]=get_aa_error('import_fields_duplication',$lang['videos']['import_divider_fields']);
		$is_field_error=1;
	}
	if (!is_array($errors))
	{
	...
	if (!in_array("user",$import_fiels_list))
	{
		...
		if (is_array($temp))
		{
			...
			foreach ($temp as $user)
			{
				if (strlen($user)>0)
				{
					...
					{
						$errors[]=get_aa_error('import_users',$lang['videos']['import_field_users'],$user);
					}
				}
			}
		}
		if ($has_users==0)
		{
			$errors[]=get_aa_error('import_users_required',$lang['videos']['import_field_users']);
		}
	}

	if ($is_make_directories<>1 && !in_array("directory",$import_fiels_list))
	{
		$errors[]=get_aa_error('import_directory_from_title',$lang['videos']['import_field_options']);
	}
	...
	if ($is_post_time_randomization==1)
	{
		...
		{
			$errors[]=get_aa_error('invalid_time_range',$lang['videos']['import_field_post_date']);
		}
	}
	...
	if ($is_post_date_randomization==1)
	{
		if (intval($_POST['post_date_randomization_option'])==0)
		{
			...
			if (intval($_POST["post_date_randomization_from_Year"])<1 ||intval($_POST["post_date_randomization_from_Month"])<1 ||intval($_POST["post_date_randomization_from_Day"])<1 ){validate_field('empty',"",$lang['videos']['import_field_post_date']);} else
			if (intval($_POST["post_date_randomization_to_Year"])<1 ||intval($_POST["post_date_randomization_to_Month"])<1 ||intval($_POST["post_date_randomization_to_Day"])<1 ){validate_field('empty',"",$lang['videos']['import_field_post_date']);} else
			if (strtotime($post_date_randomization_from)>strtotime($post_date_randomization_to))
			{
				$errors[]=get_aa_error('invalid_date_range',$lang['videos']['import_field_post_date']);
			}
		} else {
			if (validate_field('empty_int_ext',$_POST['relative_post_date_randomization_from'],$lang['videos']['import_field_post_date']))
			{
				if (validate_field('empty_int_ext',$_POST['relative_post_date_randomization_to'],$lang['videos']['import_field_post_date']))
				{
					...
					{
						$errors[]=get_aa_error('invalid_int_range',$lang['videos']['import_field_post_date']);
					}
				}
			}
		}
	...
	{
		validate_field('empty_int',$_POST['post_date_randomization_days'],$lang['videos']['import_field_post_date']);
	}
	...
	if (!is_array($errors))
	{
		...
		exec("$config[php_path] $config[project_path]/admin/background_import.php $rnd validation $lang $admin_id > /dev/null &");
...
if (isset($_POST['save_default']) && intval($_POST['import_id']) > 0)
{
	...
	$task_id = sql_insert("insert into $config[tables_prefix]background_tasks set status_id=0, type_id=50, added_date=?", date("Y-m-d H:i:s"));

	sql_pr("insert into $config[tables_prefix]background_imports set import_id=?, task_id=?, admin_id=?, status_id=0, type_id=1, threads=?, options=?, added_date=?",
...
if ($_POST['users']=='')
{
	$_POST['users']=$options['DEFAULT_USER_IN_ADMIN_ADD_VIDEO'];
}
...
{
	if ($options['DEFAULT_STATUS_IN_ADMIN_ADD_VIDEO']==0)
	{
...
$smarty->assign('list_presets',$VIDEOS_IMPORT_PRESETS);
...
{
	$smarty->assign('page_title',$lang['videos']['import_header_preview']);
} else {
	$smarty->assign('page_title',$lang['videos']['import_header_import']);
}
